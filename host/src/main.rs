use std::fs;
use anyhow::Result;
use host::{ create_zkp_age_over_18, get_age_from_jwt, create_zkp_jwt_valid };
// These constants represent the RISC-V ELF and the image ID generated by risc0-build.
// The ELF is used for proving and the ID is used for verification.
use methods::{ AGE_OVER_18_ID, JWT_VALIDATOR_ID };
...
fn main() -> Result<()> {
    ...
    let zkp = create_zkp_jwt_valid(&jwt);
    ...
    match zkp {
        Ok(zkp) => {
            ...
            zkp.verify(JWT_VALIDATOR_ID).expect(
