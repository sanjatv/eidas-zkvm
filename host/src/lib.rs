...
pub fn get_age_from_jwt(jwt: &str) -> i32 {
    let parts: Vec<&str> = jwt.split('.').collect();

    let body_b64 = parts[1];
    // base64-decode header
    let body_json_bytes = Base64UrlUnpadded::decode_vec(body_b64).expect("invalid b64 body");
    // convert bytes -> str
    let body_json_str = core::str::from_utf8(&body_json_bytes).expect("body htf8 error");
    let json: Value = serde_json::from_str(body_json_str).expect("Invalid JSON structure.");
...
pub fn verify_age_over_18(jwt: &str) -> Result<(bool, Receipt)> {
    let receipt: Receipt = create_zkp_age_over_18(jwt)?;
    let is_valid: bool = receipt.journal
        .decode()
        .expect("Failed to deserialixe journal input as bool");
    receipt.verify(AGE_OVER_18_ID).expect("Proof verification failed");
    Ok((is_valid, receipt))
}
